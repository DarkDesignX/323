...
function view(dispatch, model) {
  return div({ className: "flex flex-col gap-4 items-center" }, [
    h1({ className: "text-2xl" }, `Count: ${model.currentTime}`),
    button({ className: btnStyle, onclick: () => dispatch(MSGS.INCREASE_COUNT) }, "+ Increase"),
    button({ className: btnStyle, onclick: () => dispatch(MSGS.DECREASE_COUNT) }, "- Decrease"),
